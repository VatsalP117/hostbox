...
export function LogViewer({
  ...
  const getLogLevel = (message: string): { level: string; className: string } => {
    ...
    if (upper.includes("[ERROR]") || upper.includes("ERROR:") || upper.includes("FAILED")) {
      return { level: "ERROR", className: "text-destructive" };
    }
    if (upper.includes("[WARN]") || upper.includes("WARNING:")) {
      return { level: "WARN", className: "text-warning" };
    }
    if (upper.includes("[INFO]") || upper.includes("INFO:")) {
      return { level: "INFO", className: "text-[#52a8ff]" };
    }
    if (upper.includes("[SUCCESS]") || upper.includes("SUCCESS:")) {
      return { level: "SUCCESS", className: "text-success" };
    }
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card">
      {/* Header */}
      <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-muted/50 backdrop-blur-md">
      ...
      <div className="relative">
          ...
          <div className="p-4 font-[JetBrains_Mono] text-xs leading-relaxed">
            ...
            {lines.map((line, i) => {
              ...
              return (
                <div 
                  key={i} 
                  className="flex gap-3 hover:bg-muted/30 transition-colors rounded px-1 -mx-1"
                >
                  {/* Line Number */}
                  <span className="select-none text-neutral-700 w-8 text-right shrink-0">
                    {i + 1}
                  </span>
                  
                  {/* Timestamp */}
                  {timestamp && (
                    <span className="select-none text-neutral-700 shrink-0">
                      {timestamp}
                    </span>
                  )}
        ...
        {!autoScroll && (
          <Button
            variant="secondary"
            size="sm"
            className="absolute bottom-4 right-4 border border-border bg-muted text-foreground opacity-90 hover:bg-accent hover:opacity-100"
            onClick={() => {
              setAutoScroll(true);
              bottomRef.current?.scrollIntoView({ behavior: "smooth" });
            }}
